...
export function ReportsPage() {
  const { items, categories, transactions, suppliers, branches, inventoryBalance } = useStore();
  ...
  // Filter bar (reuses the Stock Count page's Card/Label/Select/Search
  // design). These apply before any table renders — see filteredItems,
  // filteredItemIds, and stockByItem below, all shared by every tab so no
  // tab re-implements its own copy of the filtering rules. Branch filters
  // on transactions.branch_id for Period Usage/Purchase/Stock Count (added
  // in 001_core_columns_and_balance.sql, applied inside those tabs'
  // existing transaction loops the same way Date Range already was), and
  // on inventory_balance.branch_id for the Inventory tab (see stockByItem).
  ...
  // Item Search / Category / Supplier apply here, at the input-data level,
  // before any report's own calculation runs — the calculations below are
  // untouched, they just now iterate a pre-filtered item/transaction set.
  // Supplier filtering is done once, here, at the item level (each item's
  // own supplierId) rather than per-transaction, specifically so it means
  // the same thing and produces consistent results on every report tab.
  ...
  // The real per-branch stock source of truth (public.inventory_balance),
  // never items.current_stock. All Branches -> SUM(quantity) grouped by
  // item, across every branch; a specific branch -> that branch's rows
  // only. Computed once, centrally, and reused by every place in this
  // page that needs a stock quantity (Inventory tab + the Stock by
  // Category chart), so Branch selection is never applied twice or
  // inconsistently.
  ...
  const stockFor = (itemId: string) => stockByItem.get(itemId) ?? 0;
  ...
  const periodRows = useMemo(() => {
    return filteredItems.map((i) => {
      ...
      for (const t of transactions) {
        ...
        if (branchId !== "all" && t.branchId !== branchId) continue;
  ...
  }, [filteredItems, transactions, startTs, endTs, branchId]);
  ...
  // Category/Supplier/Search apply (via filteredItems), and Branch applies
  // too, via stockFor() (public.inventory_balance) — same as every other
  // tab, per-branch when a branch is selected, summed across branches for
  // "All Branches". items.current_stock is intentionally not used here:
  // it is a convenience value, not the source of truth for branch stock.
    ...
    () =>
      filteredItems.map((i) => {
        ...
        return {
          ...
          stock,
          min: i.minStock,
          status: stock <= i.minStock ? "Low" : "OK",
        };
      }),
    ...
    <div className="space-y-6">
      ...
      <Card className="rounded-2xl border-border/70 p-5 shadow-sm">
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-4">
          ...
          <div>
            <Label className="mb-1.5 block text-sm">Supplier</Label>
          ...
          <div>
            <Label className="mb-1.5 block text-sm">Search (Code, Name, or Supplier)</Label>
