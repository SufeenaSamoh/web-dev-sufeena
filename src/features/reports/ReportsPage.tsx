...
export function ReportsPage() {
  ...
  const periodRows = useMemo(() => {
    ...
    return items.map((i) => {
      ...
      let beginning = 0, purchases = 0, adjustments = 0, usageTxn = 0;
  ...
  const totals = useMemo(() => periodRows.reduce((acc, r) => ({
    beginning: acc.beginning + r.beginning,
    purchases: acc.purchases + r.purchases,
    ending: acc.ending + r.ending,
    usage: acc.usage + r.usage,
    cost: acc.cost + r.cost,
  }), { beginning: 0, purchases: 0, ending: 0, usage: 0, cost: 0 }), [periodRows]);

  const inventoryRows = useMemo(() => items.map((i) => ({
    code: i.code, name: i.name, category: categories.find((c) => c.id === i.categoryId)?.name ?? "",
    stock: currentStock(i.id), min: i.minStock, status: currentStock(i.id) <= i.minStock ? "Low" : "OK",
  })), [items, categories, currentStock]);

  const purchaseRows = useMemo(() => transactions.filter((t) => t.type === "purchase").map((t) => ({
    date: new Date(t.date).toLocaleDateString(), item: items.find((i) => i.id === t.itemId)?.name ?? "",
    supplier: suppliers.find((s) => s.id === t.supplierId)?.name ?? "", qty: t.quantity,
    price: t.unitPrice ?? 0, total: (t.unitPrice ?? 0) * t.quantity,
  })), [transactions, items, suppliers]);

  const stockCountRows = useMemo(() => transactions.filter((t) => t.type === "adjustment").map((t) => ({
    date: new Date(t.date).toLocaleDateString(), item: items.find((i) => i.id === t.itemId)?.name ?? "",
    diff: t.quantity, remark: t.remark ?? "",
  })), [transactions, items]);

  const byCategory = categories.map((c) => ({
    name: c.name, value: items.filter((i) => i.categoryId === c.id).reduce((s, i) => s + currentStock(i.id), 0),
  }));

  const topUsage = useMemo(() => [...periodRows]
    .filter((r) => r.usage > 0)
  ...
  const exportPdf = () => { toast.success("Print dialog opened — save as PDF"); window.print(); };

  return (
    <div className="space-y-6">
      <PageHeader title="Reports · รายงาน" description="วิเคราะห์การใช้วัตถุดิบตามรอบตรวจนับ" actions={
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" onClick={() => exportCsv("period-report", periodRows)} className="rounded-xl"><Download className="mr-1 h-4 w-4" />CSV</Button>
          <Button variant="outline" onClick={() => exportCsv("period-report", periodRows)} className="rounded-xl"><FileSpreadsheet className="mr-1 h-4 w-4" />Excel</Button>
          <Button variant="outline" onClick={exportPdf} className="rounded-xl"><FileText className="mr-1 h-4 w-4" />PDF</Button>
          <Button variant="outline" onClick={printPage} className="rounded-xl"><Printer className="mr-1 h-4 w-4" />Print</Button>
        </div>
      } />
      ...
      <Card className="rounded-2xl border-border/70 p-5 shadow-sm">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
          ...
          <div>
            ...
            <Input type="date" value={start} onChange={(e) => { setStart(e.target.value); setPreset("custom"); }} className="h-11 rounded-xl" />
          </div>
          <div>
            <Label className="mb-1.5 block text-sm">สิ้นสุด (End)</Label>
            <Input type="date" value={end} onChange={(e) => { setEnd(e.target.value); setPreset("custom"); }} className="h-11 rounded-xl" />
          </div>
          <div className="flex items-end">
            <div className="w-full rounded-xl border border-border/70 bg-accent/40 p-3 text-center">
              <div className="text-[10px] uppercase tracking-widest text-muted-foreground">Usage Cost</div>
      ...
      <div className="grid gap-4 lg:grid-cols-4">
        {[
          ...
          <Card key={k.label} className="rounded-2xl border-border/70 p-4 shadow-sm">
            <div className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">{k.label}</div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="rounded-2xl border-border/70 p-4 shadow-sm">
          ...
          <div className="h-64"><ResponsiveContainer><BarChart data={topUsage} layout="vertical" margin={{ left: 40 }}>
            <CartesianGrid strokeDasharray="3 3" opacity={0.2} />
            <XAxis type="number" fontSize={11} /><YAxis type="category" dataKey="name" fontSize={11} width={100} />
            <Tooltip /><Bar dataKey="usage" fill="#1565C0" radius={[0,6,6,0]} />
          </BarChart></ResponsiveContainer></div>
        </Card>
        <Card className="rounded-2xl border-border/70 p-4 shadow-sm">
          <h3 className="mb-2 text-sm font-semibold">Stock by Category</h3>
          <div className="h-64"><ResponsiveContainer><PieChart>
            <Pie data={byCategory} dataKey="value" nameKey="name" innerRadius={50} outerRadius={95}>
      ...
      <Card className="rounded-2xl border-border/70 p-4 shadow-sm">
        <Tabs value={tab} onValueChange={setTab}>
          <TabsList className="flex w-full flex-wrap justify-start rounded-xl bg-muted p-1">
            <TabsTrigger value="period" className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow">Period Usage</TabsTrigger>
            <TabsTrigger value="inventory" className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow">Inventory</TabsTrigger>
            <TabsTrigger value="purchase" className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow">Purchase</TabsTrigger>
            <TabsTrigger value="count" className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow">Stock Count</TabsTrigger>
          </TabsList>
                ...
                <TableBody>
                  {periodRows.map((r) => (
                    <TableRow key={r.code}>
                      ...
                      <TableCell className="text-right font-semibold tabular-nums">{r.usage}</TableCell>
                      <TableCell className={`text-right tabular-nums ${r.difference > 0 ? "text-emerald-600" : r.difference < 0 ? "text-destructive" : ""}`}>
          ...
          <TabsContent value="inventory" className="mt-4">
            <div className="overflow-x-auto"><Table>
              <TableHeader><TableRow>
                ...
                <TableHead className="text-right">Stock</TableHead><TableHead className="text-right">Min</TableHead><TableHead>Status</TableHead>
              ...
              <TableBody>
                {inventoryRows.map((r) => (
                  <TableRow key={r.code}>
                    ...
                    <TableCell>{r.status === "Low"
                      ? <Badge variant="destructive">Low</Badge>
                      : <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100">OK</Badge>}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table></div>
            ...
            <div className="overflow-x-auto"><Table>
              ...
              <TableBody>
                {purchaseRows.map((r, idx) => (
                  <TableRow key={idx}>
                    <TableCell>{r.date}</TableCell><TableCell>{r.item}</TableCell><TableCell>{r.supplier}</TableCell>
          ...
          <TabsContent value="count" className="mt-4">
            <div className="overflow-x-auto"><Table>
              <TableHeader><TableRow>
                ...
                <TableHead className="text-right">Difference</TableHead><TableHead>Remark</TableHead>
              </TableRow></TableHeader>
              <TableBody>
                {stockCountRows.map((r, idx) => (
                  <TableRow key={idx}>
                    <TableCell>{r.date}</TableCell><TableCell>{r.item}</TableCell>
                    <TableCell className={`text-right tabular-nums ${r.diff > 0 ? "text-emerald-600" : r.diff < 0 ? "text-destructive" : ""}`}>
