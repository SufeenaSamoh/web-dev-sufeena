...
export function SuppliersPage() {
  const { suppliers, addSupplier, updateSupplier, deleteSupplier, transactions, settings } =
    useStore();
  ...
  const filtered = useMemo(
    () =>
      suppliers.filter((s) =>
        (s.name + s.code + s.contactPerson).toLowerCase().includes(q.toLowerCase()),
      ),
    [suppliers, q],
  );
  ...
  const openCreate = () => {
    setForm({ ...empty, code: `SUP-${String(suppliers.length + 1).padStart(3, "0")}` });
    setCreating(true);
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Suppliers"
        description="ผู้จำหน่าย · Manage vendors and delivery partners."
        actions={
              ...
              {filtered.map((s) => (
                <TableRow key={s.id}>
                  ...
                  <TableCell className="text-right tabular-nums">
                    {summaryBySupplier.get(s.id)?.orders ?? 0}
                  </TableCell>
                  <TableCell className="text-right tabular-nums font-medium">
                    {formatCurrency(summaryBySupplier.get(s.id)?.value ?? 0, settings.currency)}
                  </TableCell>
                  <TableCell>
                    {s.active ? (
