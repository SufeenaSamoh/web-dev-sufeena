...
export function SettingsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Settings"
        description="Restaurant, preferences and data."
        actions={
          <div className="flex gap-2">
            <Button variant="outline" onClick={backup} className="rounded-xl">
        ...
        <TabsContent value="profile">
          <Card className="rounded-2xl border-border/70 p-5 shadow-sm">
            ...
            <p className="mb-4 text-xs text-muted-foreground">
              These details appear across the sidebar, login page, dashboard, reports and printed
              documents.
            </p>
                ...
                <div className="mt-2 flex items-center gap-4">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      PNG or JPG, up to 2 MB. Square works best.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <TabsContent value="prefs">
          <Card className="rounded-2xl border-border/70 p-5 shadow-sm">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label>Low Stock Threshold (fallback)</Label>
                  <Input
                    type="number"
                ...
                <div>
                  <Label>Expiry Warning Threshold (Days)</Label>
                  <Input
                    type="number"
                    min={1}
                    max={180}
                    value={form.expiryWarningDays ?? 7}
                    onChange={(e) =>
                      setForm({ ...form, expiryWarningDays: Math.max(1, Number(e.target.value)) })
                    }
                    className="h-11 rounded-xl"
                  />
                  <p className="mt-1 text-[11px] text-muted-foreground">
                    Days before expiry to raise warnings.
                  </p>
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-border/50">
                <h4 className="text-xs font-semibold text-foreground uppercase tracking-wider mb-2">
                  Usage Variance Report Thresholds (เกณฑ์แจ้งเตือนการใช้จริง vs สูตร)
                </h4>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Label>Variance Diff Threshold (%)</Label>
                    <div className="relative mt-1">
                      <Input
                        ...
                        max={100}
                        step={0.5}
                        value={form.variancePercentThreshold ?? 5}
                        onChange={(e) =>
                          setForm({
                            ...form,
                            variancePercentThreshold: Math.max(0.1, Number(e.target.value)),
                          })
                        }
                    ...
                    <p className="mt-1 text-[11px] text-muted-foreground">
                      แจ้งเตือนเมื่อผลต่าง (%) เกินกว่าที่กำหนด (ค่าเริ่มต้น 5%)
                    </p>
                  </div>
                  <div>
                    <Label>Variance Value Threshold (บาท ฿)</Label>
                    <div className="relative mt-1">
                      <Input
                        type="number"
                        min={1}
                        step={10}
                        value={form.varianceValueThreshold ?? 100}
                        onChange={(e) =>
                          setForm({
                            ...form,
                            varianceValueThreshold: Math.max(1, Number(e.target.value)),
                          })
                        }
                        className="h-11 rounded-xl pr-8"
                      />
                    ...
                    <p className="mt-1 text-[11px] text-muted-foreground">
                      แจ้งเตือนเมื่อมูลค่าผลต่าง (บาท) เกินกว่าที่กำหนด (ค่าเริ่มต้น ฿100)
                    </p>
                  </div>
                </div>
              </div>
              ...
              {[
                {
                  ...
                  desc: "Ping when an item drops below its minimum.",
                },
                {
                  key: "expiring",
                  label: "Expiry warnings",
                  desc: "Warn 7 days before batch expiry.",
                },
                {
                  key: "dailyDigest",
                  label: "Daily digest email",
                  desc: "Summary of today's movement each evening.",
                },
                {
                  key: "purchaseAlert",
                  label: "Purchase confirmations",
                  desc: "Confirm every receipt from suppliers.",
                },
              ].map((n) => (
                <div
                  key={n.key}
