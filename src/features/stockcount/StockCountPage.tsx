...
export function StockCountPage() {
  const { items, categories, currentStock, addTransaction } = useStore();
  ...
  const filtered = useMemo(
    () => items.filter((i) => i.active && (cat === "all" || i.categoryId === cat) && (i.name.toLowerCase().includes(q.toLowerCase()) || i.code.toLowerCase().includes(q.toLowerCase()))),
    [items, cat, q]
  );
  ...
  const save = () => {
    ...
    if (entries.length === 0) { toast.error("กรุณาระบุจำนวนอย่างน้อย 1 รายการ"); return; }
    let adjusted = 0;
    entries.forEach(([id, actual]) => {
      const diff = actual - currentStock(id);
      if (diff !== 0) {
        addTransaction({ itemId: id, type: "adjustment", quantity: diff, date: new Date(date).toISOString(), remark: "ตรวจนับสต๊อก" });
        adjusted++;
      }
    });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<Button onClick={save} className="rounded-xl"><Save className="mr-1 h-4 w-4" />บันทึก (Save)</Button>}
      />
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          <div>
            ...
            <div className="flex h-11 items-center gap-2 rounded-xl border border-border bg-muted/50 px-3">
              ...
              <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="รหัส หรือ ชื่อวัตถุดิบ…" className="h-8 border-0 bg-transparent shadow-none focus-visible:ring-0" />
            </div>
          </div>
        </div>
      </Card>
            ...
            <Card key={i.id} className="rounded-2xl border-border/70 p-3 shadow-sm">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground">{i.code} · {i.unit}</div>
                </div>
                <div className="shrink-0 text-right">
                  <div className="text-[10px] uppercase tracking-wider text-muted-foreground">ในระบบ</div>
              ...
              <div className="mt-3 flex items-center gap-2">
                ...
                <div className={cn(
                  ...
                  diff === null ? "bg-muted text-muted-foreground" :
              ...
              {filtered.map((i) => {
                ...
                return (
                  <TableRow key={i.id}>
                    ...
                    <TableCell className="font-medium">{i.name} <span className="text-xs text-muted-foreground">({i.unit})</span></TableCell>
                    <TableCell className="text-right tabular-nums">{sys}</TableCell>
                    <TableCell>
                      <Input type="number" value={counted ?? ""} onChange={(e) => setActual(i.id, Number(e.target.value))} className="h-9 rounded-lg text-right" />
                    </TableCell>
